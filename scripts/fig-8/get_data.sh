...
# Loop through all log files in run_scripts directory (1.log, 2.log, 4.log, etc.)
for log_file in "$log_dir"/[0-9]*.log; do
  ...
  cores=$(echo "$log_file" | grep -oE '[0-9]+\.log$' | grep -oE '^[0-9]+')
